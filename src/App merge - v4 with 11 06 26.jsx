...
const SYSTEM_PROMPT = `Tu es Klea, l'assistante RH de Proelan, développée par Agnès, DRH experte avec 15 ans d'expérience dans les ESN françaises.

Proelan est une ESN française de 70 collaborateurs basée à Sophia Antipolis, spécialisée dans le conseil et les services informatiques. Les collaborateurs sont des ingénieurs et cadres en informatique, travaillant en mission chez des clients.

Tu es disponible pour tous les collaborateurs (salariés Proelan, salariés portés Astreoz/Aneemaa, freelances, salariés d'autres ESN) pour répondre à leurs questions RH du quotidien : congés, paie, CRA, facturation, process en mission, avantages, inter-contrat...

Tu apportes des réponses précises, actuelles et adaptées au contexte ESN française et à la convention collective Syntec (IDCC 1486) pour les salariés Proelan.
...
- **Congés supplémentaires d'ancienneté** : les salariés Proelan bénéficient d'**1 jour de congé payé supplémentaire tous les 5 ans** d'ancienneté
...
- **Mutuelle / Prévoyance / Tickets restaurant** : documents disponibles sur BoondManager, Agnès est l'interlocutrice pour l'adhésion ⚠️ Réservé aux salariés Proelan. La mutuelle est prise en charge à **100% par Proelan** — c'est un avantage important !
...
## Contacts clés
  - 👩‍💼 **Agnès** — DRH, interlocutrice pour : RH, formation, paie, facturation, administratif, suivi des collaborateurs et événementiel | 📧 abasuyau@proelan.fr | 📞 06 58 56 57 61 (appel, SMS, WhatsApp)
...
Commence par te présenter avec ce message exact : "Bonjour, je suis Klea, l'agent RH IA développée par Agnès pour vous servir ! 😊 ⚠️ Pour votre confidentialité, merci de ne pas partager de données personnelles dans nos échanges (nom, numéro de sécurité sociale, coordonnées bancaires...). Pour mieux vous aider, quel est votre statut chez Proelan ?" — Les boutons de profil seront affichés automatiquement par l'interface. Une fois le profil sélectionné, adapte toutes tes réponses à ce profil.`;
...
const QUICK_ACTIONS = [
  ...
  "Fin de mission & Inter-contrat",
  "Outils & Accès",
];

const QUICK_ACTIONS_DESC = {
  "Congés & RTT": "Poser des congés, compteur RTT, règles d'acquisition, lundi de Pentecôte...",
  "CRA & BoondManager": "Remplir son CRA mensuel, accès BoondManager, notes de frais...",
  "Ma mission client": "Suivi de mission, interlocuteur commercial, fin de mission...",
  "Paie & Bulletins": "Accéder à son bulletin, questions sur la paie, maintien maladie...",
  "Avantages & CSE": "Mutuelle, tickets resto, CSE, billetterie, remboursement sport...",
  "Formation & Entretiens": "Plan de formation, entretien annuel, demande de formation...",
  "Fin de mission & Inter-contrat": "Que faire en fin de mission, inter-contrat, CV Proelan...",
  "Outils & Accès": "Teams, email, BoondManager, Karburan, Edoc, Swile...",
};
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: "20px 20px 8px", display: "flex", flexDirection: "column", gap: 14 }}>
          {!started ? (
            ...
            <>
              ...
              {/* Profile selector — shown after first message, before profile is chosen */}
              {messages.length > 0 && !profile && !loading && (
                <div style={{ display: "flex", flexDirection: "column", gap: 8, padding: "4px 0 4px 36px" }}>
                  <div style={{ fontSize: 12, color: "#8a9bb0", fontFamily: "sans-serif", marginBottom: 4 }}>
                    Choisissez votre statut :
                  </div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                    {[
                      { id: "salarié", label: "👔 Salarié Proelan" },
                      { id: "porté", label: "🤝 Salarié porté (Astreoz/Aneemaa)" },
                      { id: "freelance", label: "🧑‍💻 Freelance" },
                      { id: "autre_esn", label: "🏢 Salarié d'une autre ESN" },
                    ].map((p) => (
                      <button
                        key={p.id}
                        onClick={() => {
                          setProfile(p.id);
                          sendMessage(`Mon statut est : ${p.label}`);
                        }}
                        style={{
                          padding: "8px 16px", borderRadius: 20,
                          background: "linear-gradient(135deg, rgba(201,169,110,0.15), rgba(232,201,122,0.1))",
                          border: "1px solid rgba(201,169,110,0.4)",
                          color: "#e8c97a", fontSize: 13, cursor: "pointer",
                          fontFamily: "sans-serif", transition: "all 0.2s",
                        }}
                        onMouseEnter={e => { e.currentTarget.style.background = "rgba(201,169,110,0.3)"; }}
                        onMouseLeave={e => { e.currentTarget.style.background = "linear-gradient(135deg, rgba(201,169,110,0.15), rgba(232,201,122,0.1))"; }}
                      >
                        {p.label}
                      </button>
                    ))}
        ...
        {/* Quick actions — shown only after profile is selected */}
        {started && messages.length > 0 && profile && !loading && (
          <div style={{ padding: "8px 16px", borderTop: "1px solid rgba(255,255,255,0.05)", display: "flex", gap: 6, flexWrap: "wrap" }}>
            {QUICK_ACTIONS.map((action, i) => (
              <button key={i} onClick={() => sendMessage(`J'ai une question sur : ${action}`)} style={{
